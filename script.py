...
def transform_to_paychex_template(df):
    ...
    handling Pay Component extraction, hourly vs. salaried distinctions, and
    ...
    # Extract or map core fields
    client_id = get_col(["Client ID", "Client", "Client_ID"], "")

    # Worker ID: Employee ID with Column E (index 4) fallback
    emp_id = get_col(["Employee ID", "Worker ID", "ID", "Employee_ID"], "")
    if isinstance(emp_id, str) and emp_id == "" and len(work_df.columns) >= 5:
        emp_id = work_df.iloc[:, 4]

    # Pay Component: Ensure it pulls from available component/type columns, fallback to 'Regular' or 'Overtime'
    pay_comp = get_col(
        ["Pay Component", "Component", "Earning Code", "Pay Type", "Description", "Earnings"], ""
    )
    if isinstance(pay_comp, str) and pay_comp == "":
        pay_comp = "Regular"

    hours = get_col(["Hours", "Total Hours", "Reg Hours"], 0.0)
    rate = get_col(["Rate", "Hourly Rate"], 0.0)
    units = get_col(["Units", "Point Units", "Points"], "")
    amount = get_col(["Amount", "Total", "Pay Amount", "Gross Pay"], 0.0)
    emp_name = get_col(
        ["Employee Name", "Name", "Worker Name", "Employee_Name", "Full Name"], ""
    )

    # Construct the standardized Paychex DataFrame
    new_df = pd.DataFrame()
    new_df["Client ID"] = client_id
    new_df["Worker ID"] = emp_id

    # Explicitly blank columns as required by template
    for col in [
        "Org",
        "Job Number",
        "Rate Number",
